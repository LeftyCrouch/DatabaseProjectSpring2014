...
<html>
  ...
  <body>
    <form name="delteSupplierProduct" method="post" action="deleteD.php">
      UPC<select name="UPC">
      <?php 
	...
	foreach ( $categoryData as $category ) {
      ...
      <input type="hidden" value="<? echo $supplierID ?>" name="supplierID">
      <input type="submit">
    </form>

     <?php
       $query = "SELECT UPC, Pname, price, shippingcost From (select * from suppliedby where supplierID = :supplierID ) as S join product using (UPC)";
       $stmt = $dbh->prepare($query);
       $stmt->bindParam('supplierID',$supplierID);
       $stmt->execute();
       $categoryData = $stmt->fetchAll(PDO::FETCH_OBJ);
